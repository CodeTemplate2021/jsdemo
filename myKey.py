...
if WSKEY_DEBUG in os.environ or WSKEY_MODE  # 判断调试模式变量
    ...
    logger.debug(nDEBUG模式开启!n)  # 消息输出
else
...
try
    ...
    logger.info(str(e) + n缺少requests模块, 请执行命令：pip3 install requestsn)  # 日志输出
    sys.exit(1)  # 退出脚本
...
def genJDUA()
    ...
    ua = 'jdapp;android;11.1.4;;;appBuild98176;ef1;ep{hdidJM9F1ywUPwflvMIpYPok0tt5k9kW4ArJEU3lfLhxBqw=,ts%s,ridx-1,cipher{svCJS=,ad%s,od%s,ovCzO=,ud%s},ciphertype5,version1.2.0,appnamecom.jingdong.app.mall};Mozilla5.0 (Linux; Android 12; M2102K1C BuildSKQ1.220303.001; wv) AppleWebKit537.36 (KHTML, like Gecko) Version4.0 Chrome97.0.4692.98 Mobile Safari537.36' % (st, aid, oaid, aid)
    return ua
...
def genParams()
    ...
    body = '{tohttps%3a%2f%2fplogin.m.jd.com%2fjd-mlogin%2fstatic%2fhtml%2fappjmp_blank.html}'
    ...
    params = {
        ...
        'eid' 'eidAcef08121fds9MoeSDdMRQ1aUTyb1TyPr2zKHk5Asiauw+KWvS1Ben1cH6N0UnBd7lNM50XEa2kfCcA2wwThkxZc1MuCNtfUoAMGBqadgres4BU',
        'ef' '1',
        'ep' ep,
        'st' st,
        'sign' sign,
...
def get_qltoken(username, password, twoFactorSecret)  # 方法 用于获取青龙 Token
    ...
    if twoFactorSecret
        try
            ...
            logger.info(TOTP异常)
            sys.exit(1)
        url = ql_url + apiuserlogin  # 设置青龙地址 使用 format格式化自定义端口
        body = {
            'username' username,
            'password' password
        }  # HTTP请求载荷
        ...
        try
            ...
            if res.status_code == 200 and res.json()[code] == 420
                url = ql_url + 'apiusertwo-factorlogin'
                body = {
                    'username' username,
                    'password' password,
                    'code' twoCode
        ...
        try
            ...
            url = ql_url + 'apilogin'  # 设置青龙地址 使用 format格式化自定义端口
            try
                res = requests.post(url=url, headers=headers, json=body)  # 使用 requests模块进行 HTTP POST请求
...
def ql_login() - str  # 方法 青龙登录(获取Token 功能同上)
    path = 'qlconfigauth.json'  # 设置青龙 auth文件地址
    if not os.path.isfile(path)
        path = 'qldataconfigauth.json'  # 尝试设置青龙 auth 新版文件地址
    if os.path.isfile(path)  # 进行文件真值判断
        ...
        if token == ''  # 判断 Token是否为空
            ...
            url = ql_url + 'apiuser'  # 设置URL请求地址 使用 Format格式化端口
            headers = {
                'Authorization' 'Bearer {0}'.format(token),
                'User-Agent' 'Mozilla5.0 (Windows NT 10.0; Win64; x64) AppleWebKit537.36 (KHTML, like Gecko) Chrome94.0.4606.71 Safari537.36 Edg94.0.992.38'
            }  # 设置用于 HTTP头
...
def check_ck(ck) - bool  # 方法 检查 Cookie有效性 使用变量传递 单次调用
    ...
    if WSKEY_UPDATE_HOUR in os.environ  # 判断 WSKEY_UPDATE_HOUR是否存在于环境变量
        ...
        if nowTime - updatedAt = (updateHour  60  60) - (10  60)  # 判断时间操作
            logger.info(str(pin) + ;即将到期或已过期n)  # 标准日志输出
            ...
            logger.info(str(pin) + ;未到期，{0}时{1}分后更新n.format(hour, minute))  # 标准日志输出
            return True  # 返回 Bool类型 True
    elif WSKEY_DISCHECK in os.environ
        logger.info(不检查账号有效性n--------------------n)  # 标准日志输出
        return False  # 返回 Bool类型 False
    else
        url = 'httpsme-api.jd.comuser_newinfoGetJDUserInfoUnion'  # 设置JD_API接口地址
        headers = {
            'Cookie' ck,
            'Referer' 'httpshome.m.jd.commyJdhome.action',
            'user-agent' genJDUA()
        }  # 设置 HTTP头
        try
            ...
            if res.status_code == 200  # 判断 JD_API 接口是否为 200 [HTTP_OK]
                try
                    ...
                    return False
                if code == 0  # 判断 code值
                    logger.info(str(pin) + ;状态正常n)  # 标准日志输出
                    return True  # 返回 Bool类型 True
                else
                    logger.info(str(pin) + ;状态失效n)
                    return False  # 返回 Bool类型 False
            else
...
def getToken(wskey)  # 方法 获取 Wskey转换使用的 Token 由 JD_API 返回 这里传递 wskey
    # try
    #     url = str(base64.b64decode(url_t).decode()) + 'apigenToken'  # 设置云端服务器地址 路由为 genToken
    ...
    url = 'httpsapi.m.jd.comclient.action'  # 设置 URL地址
...
def appjmp(wskey, tokenKey)  # 方法 传递 wskey & tokenKey
    ...
    if tokenKey == 'xxx'  # 判断 tokenKey返回值
        logger.info(str(wskey) + ;疑似IP风控等问题 默认为失效n--------------------n)  # 标准日志输出
        # return False, wskey  # 返回 - False[Bool], Wskey
        return False  # 返回 - False[Bool], Wskey
    headers = {
        'User-Agent' genJDUA(),
        'accept' 'accepttexthtml,applicationxhtml+xml,applicationxml;q=0.9,imageavif,imagewebp,imageapng,;q=0.8,applicationsigned-exchange;v=b3;q=0.9',
        'x-requested-with' 'com.jingdong.app.mall'
    }  # 设置 HTTP头
    params = {
        'tokenKey' tokenKey,
        'to' 'httpsplogin.m.jd.comjd-mloginstatichtmlappjmp_blank.html'
    }  # 设置 HTTP_URL 参数
    url = 'httpsun.m.jd.comcgi-binappappjmp'  # 设置 URL地址
    try
        ...
        logger.info(JD_appjmp 接口错误 请重试或者更换IPn)  # 标准日志输出
        ...
        try
            ...
            logger.info(JD_appjmp提取Cookie错误 请重试或者更换IPn)  # 标准日志输出
            logger.info(str(err))  # 标准日志输出
            # return False, wskey  # 返回 - False[Bool], Wskey
            return False  # 返回 - False[Bool], Wskey
        else
            if 'fake' in pt_key  # 判断 pt_key中 是否存在fake
                logger.info(str(wskey) + ;WsKey状态失效n)  # 标准日志输出
                # return False, wskey  # 返回 - False[Bool], Wskey
                return False  # 返回 - False[Bool], Wskey
            else
                logger.info(str(wskey) + ;WsKey状态正常n)  # 标准日志输出
...
def ql_api(method, api, body=None) - dict
    ...
    for retry_count in range(3)
        try
            ...
            logger.debug(str(err))
            logger.info(fn青龙{api}接口错误，重试次数：{retry_count + 1})
            continue
        else
            return res
    logger.info(fn青龙{api}接口多次重试仍然失败)
    sys.exit(1)
...
def ql_enable(eid)  # 方法 青龙变量启用 传递值 eid
    ...
    if res['code'] == 200  # 判断返回值为 200
        logger.info(n账号启用n--------------------n)  # 标准日志输出
        return True
    else
        logger.info(n账号启用失败n--------------------n)  # 标准日志输出
        return False
...
def ql_disable(eid)  # 方法 青龙变量禁用 传递 eid
    ...
    res = ql_api(PUT, api, body)
    if res['code'] == 200  # 判断返回值为 200
        logger.info(n账号禁用成功n--------------------n)  # 标准日志输出
    else
        logger.info(n账号禁用失败n--------------------n)  # 标准日志输出
...
def ql_insert(i_ck)  # 方法 插入新变量
    ...
    if res['code'] == 200  # 判断返回值为 200
        logger.info(n账号添加完成n--------------------n)  # 标准日志输出
    else
        logger.info(n账号添加失败n--------------------n)  # 标准日志输出


# def cloud_info()  # 方法 云端信息
#     url = str(base64.b64decode(url_t).decode()) + 'apicheck_api'  # 设置 URL地址 路由 [check_api]
...
#             logger.info(n获取云端参数超时, 正在重试! + str(i))  # 标准日志输出
#             time.sleep(1)  # 休眠 1秒
#             continue  # 循环继续
...
#             continue  # 循环继续
#         except Exception as err
#             logger.info(n未知错误云端, 退出脚本!)  # 标准日志输出
#             logger.debug(str(err))  # 调试日志输出
#             sys.exit(1)  # 脚本退出
#         else
#             try
...
#             logger.info(str(info[url_list.index(i)]) +  Server Check OKn--------------------n)  # 标准日志输出
#             return i  # 返回 -i
#     logger.info(n云端地址全部失效, 请检查网络!)  # 标准日志输出
#     ql_send('云端地址失效. 请联系作者或者检查网络.')  # 推送消息
#     sys.exit(1)  # 脚本退出


def check_port()  # 方法 检查变量传递端口
    logger.info(n--------------------n)  # 标准日志输出
    ...
    if ql_check(port)  # 调用方法 [ql_check] 传递 [port]
        ...
        logger.info(
            str(port) + 端口检查失败, 如果改过端口, 请在变量中声明端口 n在config.sh中加入 export QL_PORT=端口号)  # 标准日志输出
        logger.info(n如果你很确定端口没错, 还是无法执行, 在GitHub给我发issusn--------------------n)  # 标准日志输出
        sys.exit(1)  # 脚本退出


if __name__ == '__main__'  # Python主函数执行入口
    port = check_port()  # 调用方法 [check_port]  并赋值 [port]
    ql_url = f'http127.0.0.1{port}'
    ...
    for ws in wslist  # wslist变量 for循环  [wslist - ws]
        ...
        if pin in wspin  # 判断 pin 是否存在于 [wspin]
            ...
            if return_serch  # bool True 搜索到账号
                ...
                if not check_ck(jck)  # bool False 判定 JD_COOKIE 有效性
                    for count in range(tryCount)  # for循环 [tryCount]
                        ...
                        if count  tryCount  # 判断循环次
                            logger.info({0} 秒后重试，剩余次数：{1}n.format(sleepTime, tryCount - count))  # 标准日志输出
                ...
                logger.info(n新wskeyn)  # 标准日志分支
                return_ws = getToken(ws)  # 使用 WSKEY 请求获取 JD_COOKIE bool jd_ck
                if return_ws  # 判断 (return_ws[0]) 类型 [Bool]
                    logger.info(wskey转换成功n)  # 标准日志输出
                    ql_insert(return_ws)  # 调用方法 [ql_insert]
            logger.info(f暂停{sleepTime}秒n)  # 标准日志输出
            time.sleep(sleepTime)  # 脚本休眠
        else
            logger.info(WSKEY格式错误n--------------------n)  # 标准日志输出
    logger.info(执行完成n--------------------)  # 标准日志输出
    sys.exit(0)  # 脚本退出
    # Enjoy
